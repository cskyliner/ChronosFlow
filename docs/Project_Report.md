...
5. **课表**：  
支持excel导入（北京大学春秋季课表已经过验证），尽可能减少手动操作，导入课程表时候会提取期末考试时间和上课地点等信息到日程备注中(即从北大选课平台导出课程表的excel文件格式)\
...
**刘昕泽**[James-Relu](https://github.com/James-Relu)：MainWindow页面框架与页面切换，创建事件界面，日程界面，日历界面搜索栏，各页面美化，系统深、浅主题适配，壁纸添加

**王思明**[Uo-sm](https://github.com/Uo-sm)：系统托盘，通知提醒，悬浮窗，设置界面、周视图界面设计

**左若麟**[cskyliner](https://github.com/cskyliner)：事件类设计，后端数据处理以及前后端信号链接，日历、热力图界面框架实现，课表导入，AI助手

...
class TimeAxisItem(QGraphicsRectItem):
	...
	def __init__(self, rect, time_str):
		...
		self.background_color = palette.color(QPalette.Base)  # 背景色（适配主题）
		self.text_color = palette.color(QPalette.Text)  # 文字颜色

		# TODO
		self.setPen(QPen(self.background_color))
...
class WeekView(QWidget):
	...
	def __init__(self):
		...
		palette = QApplication.palette()  # 获取主题
		self.background_color = palette.color(QPalette.Button)  # 背景色（适配主题）
		self.text_color = palette.color(QPalette.Text)  # 文字颜色
	...
	def setup_time_axis(self):
		...
		bg_item.setBrush(self.background_color)
	...
	def setup_day_headers(self):
		...
		for i, date in enumerate(self.dates):
			...
			header.setBrush(self.background_color)
	...
	def setup_day_columns(self):
		...
		for i in range(7):
			...
			for h in range(self.time_slot_count):
				...
				# 绑定点击事件
				cell.mousePressEvent = lambda event, d=self.dates[i], _h=h + self.start_hour:self.handle_time_click(d, _h, event)
	...
	def add_schedule_item(self, event: ActivityEvent):
		...
		# 计算局部坐标下的 y 和高度
			...
			log.info(
				f"添加的日程:{event.title} (weekday, start_hour) = {key} 对应的时间格子位置x:{cell.x()}, y:{cell.y()}\
